...
import { lightningService } from '../index';
 ...
 */
router.post('/invoice', async (req: express.Request, res: express.Response) => {
  try {
    const { amount, memo, expiry, private: isPrivate }: CreateInvoiceRequest = req.body;
 ...
 */
router.post('/pay', async (req: express.Request, res: express.Response) => {
  try {
    const { invoice, maxFee, timeout }: PayInvoiceRequest = req.body;
 ...
 */
router.get('/invoice/:paymentHash', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const invoice = await lightningService.lookupInvoice(paymentHash);
...
router.get('/balance', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const balance = await lightningService.getChannelBalance();
...
router.get('/wallet', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const balance = await lightningService.getWalletBalance();
...
/**
 * GET /api/lightning/transactions
...
router.get('/transactions', async (req: express.Request, res: express.Response) => {
  try {
    ...
    const transactions = await lightningService.listTransactions();
 ...
 */
router.get('/node', async (req: express.Request, res: express.Response) => {
  try {
...
/**
 * POST /api/lightning/connect
 * Connect to a peer
 */
router.post('/connect', async (req: express.Request, res: express.Response) => {
  try {
    ...
    await lightningService.connectPeer(pubKey, host, parseInt(port, 10));
 ...
 * Open a channel
 */
router.post('/channel/open', async (req: express.Request, res: express.Response) => {
  try {
    ...
    await lightningService.openChannel(
      pubKey,
      parseInt(localFundingAmount, 10),
      pushSat ? parseInt(pushSat, 10) : undefined,
      isPrivate || false
    );
...
/**
 * POST /api/lightning/channel/close
 * Close a channel
 */
router.post('/channel/close', async (req: express.Request, res: express.Response) => {
  try {
...
router.get('/health', async (req: express.Request, res: express.Response) => {
  try {
    const isHealthy = await lightningService.checkConnectivity();
